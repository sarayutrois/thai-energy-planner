...
import { calculateBillAfterSolar, type ExportPolicy, type SolarGenerationIntervalInput } from "./solar-engine.js";

export type ThaiBillingSegment = Extract<CustomerSegment, "residential" | "small_business">;
...
export function calculateMonthlyNormalBill(input: MonthlyBillCalculationInput): TariffCalculationResult {
...
export function calculateMonthlyTouBill(input: TouBillCalculationInput): TariffCalculationResult {
  const tariffVersion = getOfficialThaiTariff({
    authority: input.authority,
    customerSegment: input.customerSegment,
    meterMode: "tou",
...
export function compareNormalVsTou(input: TouBillCalculationInput): NormalVsTouComparison {
  ...
  const touSavingsThb = round(new Decimal(normalGrandTotalThb).minus(touGrandTotalThb), 2);
  const touSavingsPercent = normalGrandTotalThb > 0 ? round(new Decimal(touSavingsThb).div(normalGrandTotalThb).mul(100), 2) : 0;
...
export function simulateIntervalSavings(input: IntervalSavingsSimulationInput): IntervalSavingsSimulationResult {
  const intervals = input.intervals.map((interval) => ({ ...interval })).sort((a, b) => a.timestamp.localeCompare(b.timestamp));
  const offsets = new Map(input.offsetIntervals.map((interval) => [interval.timestamp, new Decimal(interval.energyKwh)]));
  const reducedIntervals = intervals.map((interval) => {
    const reducedEnergy = Decimal.max(new Decimal(interval.energyKwh).minus(offsets.get(interval.timestamp) ?? zero), zero);
    return {
      ...interval,
      energyKwh: round(reducedEnergy, 6),
      powerKw: interval.powerKw === undefined ? undefined : round(Decimal.max(new Decimal(interval.powerKw).minus(offsets.get(interval.timestamp) ?? zero), zero), 6)
    };
  });
  ...
  const normalSavingsThb = round(new Decimal(before.normalGrandTotalThb).minus(after.normalGrandTotalThb), 2);
  const touSavingsThb = round(new Decimal(before.touGrandTotalThb).minus(after.touGrandTotalThb), 2);
...
export function calculateSolarROI(input: SolarRoiInput): SolarRoiResult {
  ...
  const annualNetBenefit = annualSavings.plus(annualExportRevenue).minus(annualOperatingCost);
  ...
  return {
    ...
    simplePaybackYears: annualNetBenefit.gt(0) ? round(new Decimal(input.systemCostThb).div(annualNetBenefit), 2) : null
  };
}

export function estimateMonthlyKwhFromBill(input: MonthlyBillCalculationInput & { monthlyBillThb: number }): number {
  ...
  for (let index = 0; index < 48; index += 1) {
    ...
    const bill = Number(calculateMonthlyNormalBill({ ...input, energyKwh: mid }).grandTotal);
...
export function estimateSolarFromMonthlyBill(input: MonthlyBillSolarEstimateInput): MonthlyBillSolarEstimateResult {
  ...
  const recommendedSystemSizeKwp = round(input.systemSizeKwp ?? clamp(estimatedMonthlyKwh / 115, 1, 20), 1);
  const capexThb = round(new Decimal(recommendedSystemSizeKwp).mul(input.capexPerKwpThb ?? 42000), 2);
  ...
  const exportPolicy: ExportPolicy = {
    ...
    notes: "Screening estimate assumes no export revenue unless the detailed solar analysis enables it."
  };
...
export function inferThaiAuthorityFromProvince(province: string): Authority {
  ...
  return normalized.includes("bangkok") || normalized.includes("กรุงเทพ") || normalized.includes("นนทบุรี") || normalized.includes("สมุทรปราการ")
    ? "MEA"
    : "PEA";
}
...
export function buildEstimatedMonthlyLoadProfile(input: {
  ...
  const dailyWeight = weights.reduce((sum, item) => sum.plus(item.weight), zero);
  ...
  for (let day = 1; day <= days; day += 1) {
    ...
    for (const item of weights) {
      const energy = dailyWeight.gt(0) ? dailyKwh.mul(item.weight).div(dailyWeight) : zero;
...
export function buildEstimatedMonthlySolarProfile(input: {
  ...
  const [year = "2026", month = "07"] = billDate.split("-");
  const days = new Date(Number(year), Number(month), 0).getDate();
  const monthlySpecificYield = [112, 118, 126, 124, 118, 108, 104, 106, 110, 112, 108, 106][Number(month) - 1] ?? 110;
  const monthlyGeneration = new Decimal(input.systemSizeKwp).mul(monthlySpecificYield).mul(0.88);
  ...
  for (let day = 1; day <= days; day += 1) {
    ...
    for (const item of shape) {
      const generation = totalWeight.gt(0) ? dailyGeneration.mul(item.weight).div(totalWeight) : zero;
      intervals.push({
        timestamp: localDateMinuteToIso(date, item.minuteOfDay),
...
function buildTouBucketIntervals(input: { billDate: string; peakKwh: number; offPeakKwh: number }) {
  const peakDate = findWeekdayDate(input.billDate);
  return [
    { timestamp: `${peakDate}T10:00:00+07:00`, energyKwh: String(Math.max(0, input.peakKwh)) },
    { timestamp: `${peakDate}T23:00:00+07:00`, energyKwh: String(Math.max(0, input.offPeakKwh)) }
  ];
}
...
function findWeekdayDate(date: string) {
  ...
  for (let day = 1; day <= 7; day += 1) {
    const candidate = new Date(Date.UTC(Number(year), Number(month) - 1, day, 12, 0, 0));
    const weekday = candidate.getUTCDay();
    if (weekday >= 1 && weekday <= 5) return `${year}-${month}-${String(day).padStart(2, "0")}`;
  }
...
function buildUsageWeights(shape: UsageShape, intervalMinutes: 15 | 30 | 60) {
  ...
  for (let minute = 0; minute < 24 * 60; minute += intervalMinutes) {
    ...
    if (shape === "day") weight = hour >= 8 && hour < 18 ? new Decimal(1.7) : new Decimal(0.45);
    if (shape === "night") weight = hour >= 18 && hour < 23 ? new Decimal(2.2) : hour < 6 ? new Decimal(0.8) : new Decimal(0.45);
    if (shape === "both") weight = hour >= 9 && hour < 22 ? new Decimal(1.25) : new Decimal(0.65);
...
function buildSolarWeights(intervalMinutes: 15 | 30 | 60) {
  ...
  for (let minute = 0; minute < 24 * 60; minute += intervalMinutes) {
    ...
    weights.push({ minuteOfDay: minute, weight: new Decimal(Math.sin(Math.PI * progress)).toDecimalPlaces(9) });
  }
  return weights;
}
