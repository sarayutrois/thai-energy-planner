...
function bangkokDateIso(date: string, hour: number) {
  ...
  return new Date(
    Date.UTC(year ?? 2026, (month ?? 1) - 1, day ?? 1, hour - 7, 0, 0),
  ).toISOString();
}
...
describe("phase 6 battery dispatch engine", () => {
  it("validates battery configuration boundaries", () => {
    expect(
      validateBatteryConfig({ ...batteryConfig, usableCapacityKwh: 11 }),
    ).toContain("usableCapacityKwh cannot exceed capacityKwh");
  ...
  it("charges solar surplus with efficiency and respects charge power and max SOC", () => {
    const result = simulateBatteryDispatch({
      ...
      config: {
        ...
        dispatchStrategy: "SOLAR_SELF_CONSUMPTION",
        chargePowerKw: 2,
      },
    });
    ...
    expect(result.selfConsumptionAfterRatio).toBeGreaterThan(
      result.selfConsumptionBeforeRatio,
    );
  ...
  it("does not charge beyond the configured usable battery window", () => {
    const result = simulateBatteryDispatch({
      loadIntervals: load([[0, 12, 1]]),
      solarIntervals: solar([[0, 12, 10]]),
      config: {
    ...
    expect(result.intervals[0]?.socAfterKwh).toBe(5);
    expect(result.intervals[0]?.chargeFromSolarKwh).toBe(4.444444);
  });
  ...
  it("uses tariff-engine period classification for TOU arbitrage and holidays", () => {
    const holiday = simulateBatteryDispatch({
      loadIntervals: [
        {
          timestamp: bangkokDateIso("2026-01-01", 10),
          energyKwh: 0,
          powerKw: 0,
        },
        {
          timestamp: bangkokDateIso("2026-01-01", 11),
          energyKwh: 0,
          powerKw: 0,
        },
      ],
  ...
  it("calculates bill before/after, financial metrics, and recommendation traces", () => {
    ...
    expect(result.dispatch.gridImportAfterKwh).toBeLessThan(
      result.dispatch.gridImportBeforeKwh,
    );
    expect(result.financial.billAfterBatteryThb).toBeLessThan(
      result.financial.billBeforeBatteryThb,
    );
    expect(result.financial.costPerUsableKwh).toBeGreaterThan(0);
    expect(
      result.recommendations.some(
        (item) => item.type === "not_financial_backup_value",
      ),
    ).toBe(true);
  });
  ...
  it("keeps the energy trace auditable for the demo analysis", () => {
    ...
    expect(Math.abs(result.dispatch.energyBalanceDeltaKwh)).toBeLessThan(
      result.dispatch.totalLoadKwh,
    );
...
describe("phase 6 EV load generation", () => {
  ...
  it("keeps off-peak charging inside tariff off-peak periods", () => {
    ...
    expect(
      profile.intervals.every((interval) => interval.periodType === "off_peak"),
    ).toBe(true);
  });
  ...
  it("warns when the charging window cannot finish before departure", () => {
    ...
    expect(profile.outsideChargingKwh).toBeGreaterThan(0);
    expect(profile.outsideChargingCostThb).toBeGreaterThan(0);
  });
  ...
  it("uses solar surplus charging when the vehicle is home during daytime", () => {
    ...
    expect(
      profile.intervals.every((interval) => interval.gridEnergyKwh === 0),
    ).toBe(true);
  });
  ...
  it("compares strategies and picks a lower-cost strategy than immediate when available", () => {
    ...
    const immediate = comparison.scenarios.find(
  ...
  it("runs demo EV comparison and links the integration path to tariff bills", () => {
    ...
    expect(
      comparison.bestChargingStrategy.billAfterEv.grandTotal,
    ).toBeDefined();
    expect(
      comparison.bestChargingStrategy.profile.intervals.length,
    ).toBeGreaterThan(0);
  });
