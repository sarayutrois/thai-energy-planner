...
import { validateTariffStatusTransition, validateTierOverlap, validateTouPeriodOverlap } from "./versioning.js";
...
import { validateFtPeriodOverlap, validateTariffForPublish } from "./versioning.js";

describe("Tariff Publish Validations", () => {
  it("validate Ft period overlap", () => {
    const validFt = [
      { effectiveFrom: new Date("2025-01-01"), effectiveTo: new Date("2025-04-30") },
    ...
    const negativeFtRate = [
      { effectiveFrom: new Date("2025-01-01"), effectiveTo: null, rateThbPerKwh: -0.1 }
    ];
    expect(validateFtPeriodOverlap(negativeFtRate)).toBe(false);

    const reversedDates = [
      { effectiveFrom: new Date("2025-05-01"), effectiveTo: new Date("2025-01-01") }
    ];
