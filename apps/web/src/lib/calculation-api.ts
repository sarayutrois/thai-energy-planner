...
const solarProfileSchema = z.enum([
  "evening_home",
  "daytime_home",
  "daytime_shop",
]);
...
export const estimateRequestSchema = z
  ...
  .refine(
    (data) =>
      data.monthlyBillThb !== undefined ||
      (data.monthlyBills !== undefined && data.monthlyBills.length > 0),
    {
      message:
        "Either monthlyBillThb or monthlyBills must be provided and non-empty",
      path: ["monthlyBillThb"],
    },
  )
  .refine(
...
function processMonthlyBills(
  monthlyBills: { month: string; billThb: number }[],
) {
...
export function runEstimateApiCalculation(
  request: EstimateApiRequest,
): EstimateApiPayload {
  const { authority, customerSegment, warnings } = resolveTariffScope(
    request.province,
    request.propertyType,
  );

  const inputBills =
    request.monthlyBills && request.monthlyBills.length > 0
      ? request.monthlyBills
      : [
          {
            month: request.billDate.slice(0, 7),
            billThb: request.monthlyBillThb!,
          },
        ];

  const { filledBills, processedMonths, average } =
    processMonthlyBills(inputBills);

  const finalWarnings = [...warnings];
  if (request.monthlyBills && request.monthlyBills.length < 12) {
    finalWarnings.push(
      "Missing months in monthlyBills were averaged and padded to complete a 12-month period.",
    );
  }
  ...
  const calculation = estimateSolarFromMonthlyBill({
    ...
    ...(request.systemSizeKwp === undefined
      ? {}
      : { systemSizeKwp: request.systemSizeKwp }),
    ...(request.capexPerKwpThb === undefined
      ? {}
      : { capexPerKwpThb: request.capexPerKwpThb }),
  });
...
export function runSolarAnalyzeApiCalculation(
  ...
  const authority =
    request.authority ?? inferThaiAuthorityFromProvince(request.province);
  ...
  const scaleFactors: number[] | undefined = (() => {
    if (request.monthlyBills && request.monthlyBills.length > 0) {
      const { filledBills, average } = processMonthlyBills(
        request.monthlyBills,
      );
...
function resolveTariffScope(
  ...
  if (propertyType === "factory") {
    warnings.push(
      "Factory/large-building tariffs are not implemented yet; this screening estimate uses small business low-voltage rates.",
    );
  }
