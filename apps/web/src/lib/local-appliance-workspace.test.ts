...
import {
  maxStoredAppliances,
  parseStoredApplianceWorkspace,
} from "./local-appliance-workspace";
...
describe("stored appliance workspace validation", () => {
  ...
  it("rejects invalid schedules, ambiguous modes and oversized workspaces", () => {
    expect(
      parseStoredApplianceWorkspace({ ...workspace, mode: "empty" }),
    ).toBeNull();
    expect(
      parseStoredApplianceWorkspace({ ...workspace, appliances: [] }),
    ).toBeNull();
    expect(
      parseStoredApplianceWorkspace({
        ...workspace,
        appliances: [{ ...appliance, power: 0 }],
      }),
    ).toBeNull();
    expect(
      parseStoredApplianceWorkspace({
