...
export function normalizeScenarioProfile(
  value: string | undefined,
): ScenarioProfileKey {
  return scenarioProfileOptions.some((option) => option.value === value)
    ? (value as ScenarioProfileKey)
    : "evening_home";
}
...
export function getScenarioDemo(
  ...
  context: {
    ...
    source?: "bill" | "demo" | undefined;
  } = {},
): ScenarioComparisonResult {
  const input = createDemoScenarioInput(profile, { meterSwitchingCostThb });
  const billDate = "2026-07-01";
  input.intervals = rebaseIntervalsToBangkokBillDate(input.intervals, billDate);
  const customerSegment =
    profile === "daytime_shop" ? "small_business" : "residential";
  const demoProfileKwh = input.intervals.reduce(
    (sum, interval) => sum + interval.energyKwh,
    0,
  );
  const monthlyEnergyKwh =
    context.monthlyKwh && context.monthlyKwh > 0
      ? context.monthlyKwh
      : demoProfileKwh * (30 / 7);
  ...
  if (context.billMonthCount !== undefined)
    input.billMonthCount = context.billMonthCount;
...
function rebaseIntervalsToBangkokBillDate(
  intervals: ReturnType<typeof createDemoScenarioInput>["intervals"],
  billDate: string,
) {
  const firstTimestamp = intervals[0]?.timestamp;
  if (!firstTimestamp) return intervals;

  const [year = 2026, month = 1, day = 1] = billDate.split("-").map(Number);
  const firstTime = new Date(firstTimestamp).getTime();
  if (!Number.isFinite(firstTime)) return intervals;

  const bangkokStartUtc = Date.UTC(year, month - 1, day, -7, 0, 0);
  return intervals.map((interval) => {
    const currentTime = new Date(interval.timestamp).getTime();
    if (!Number.isFinite(currentTime)) return interval;
    return {
      ...interval,
      timestamp: new Date(
        bangkokStartUtc + (currentTime - firstTime),
      ).toISOString(),
    };
  });
}
