        ...
        <div className="grid gap-5">
          ...
          <details className="rounded-lg border border-border bg-card">
            ...
            <div className="border-t border-border p-5">
              <div className="grid gap-3 text-sm leading-6 text-muted-foreground"><p><strong className="text-foreground">Version ID:</strong> {normalResult.tariffVersionId}</p><p><strong className="text-foreground">Source snapshot:</strong> {normalResult.tariffSnapshot.capturedAt}</p><p><strong className="text-foreground">Calculation config:</strong> {normalResult.tariffVersionLabel}</p><p><strong className="text-foreground">Verification timestamp:</strong> {normalResult.verifiedAt ?? "ไม่มี"}</p><p><strong className="text-foreground">Tariff structure:</strong> {normalResult.tariffSnapshot.tariffVersion.meterMode}</p><p><strong className="text-foreground">Raw rates:</strong> {normalResult.tariffSnapshot.tariffVersion.energyRateTiers.map((tier) => `${tier.rateThbPerKwh} บาท/kWh`).join(", ")}</p></div>
            </div>
          </details>
