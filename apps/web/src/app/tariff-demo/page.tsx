...
import {
  getOfficialTariffDemo,
  type TariffDemoSearchParams,
} from "@/lib/tariff-demo";
...
export default async function TariffDemoPage({
  ...
  const {
    ...
    touResult,
    warnings,
  } = getOfficialTariffDemo(params);
  const tariffReady =
    normalResult.tariffStatus === "published" &&
    Boolean(getFtPeriod(normalResult, billDate));
      ...
      <section className="border-b border-border bg-white/78">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-4 py-8 md:px-6 lg:py-10">
          <div className="flex flex-wrap gap-2">
            ...
            <Badge variant={tariffReady ? "success" : "warning"}>
              {tariffReady ? "ตรวจสอบแล้ว" : "ข้อมูลอัตรายังไม่พร้อม"}
            </Badge>
          </div>
          <div className="max-w-3xl space-y-3">
            <h1 className="text-3xl font-semibold tracking-normal md:text-4xl">
              ตรวจสอบอัตราค่าไฟที่ใช้คำนวณ
            </h1>
            <p className="leading-7 text-muted-foreground">
              ตรวจสอบอัตราค่าไฟที่ใช้คำนวณ พร้อมแหล่งอ้างอิงอัตราค่าไฟฐานและค่า
              Ft แยกกันอย่างชัดเจน
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-5 px-4 py-8 md:px-6 lg:grid-cols-[360px_1fr] lg:py-10">
        <Card>
          ...
          <CardContent>
            ...
            {!tariffReady ? (
              <div className="mt-6 rounded-md border border-warning bg-warning/10 p-4 text-sm leading-6 text-warning-foreground">
                ข้อมูลอัตราค่าไฟหรือค่า Ft สำหรับวันที่เลือกยังไม่พร้อมตรวจสอบ
                ระบบจะไม่ใช้ข้อมูลนี้สร้างผลลัพธ์โดยอัตโนมัติ
              </div>
            ) : null}
          </CardContent>
        </Card>
        ...
        <div className="grid gap-5">
          ...
          <ResultPanel
            icon={ReceiptText}
            result={normalResult}
            title="รายละเอียดค่าไฟอัตราปกติ"
          />
          <ResultPanel
            icon={Clock3}
            result={touResult}
            title="รายละเอียดค่าไฟตามช่วงเวลา (TOU)"
          />
          ...
          <details className="rounded-lg border border-border bg-card">
            ...
            <div className="border-t border-border p-5">
              <div className="grid gap-3 text-sm leading-6 text-muted-foreground">
                <p>
                  <strong className="text-foreground">รหัสเวอร์ชัน:</strong>{" "}
                  {normalResult.tariffVersionId}
                </p>
                <p>
                  <strong className="text-foreground">สำเนาแหล่งข้อมูล:</strong>{" "}
                  {normalResult.tariffSnapshot.capturedAt}
                </p>
                <p>
                ...
                <p>
                  ...
                  {normalResult.verifiedAt ?? "ไม่มี"}
                </p>
                <p>
                  <strong className="text-foreground">
                    โครงสร้างอัตราค่าไฟ:
                  </strong>{" "}
                  {normalResult.tariffSnapshot.tariffVersion.meterMode}
                </p>
                <p>
                  <strong className="text-foreground">
                    อัตราค่าไฟรายละเอียด:
                  </strong>{" "}
                  {normalResult.tariffSnapshot.tariffVersion.energyRateTiers
                    .map((tier) => `${formatRate(tier.rateThbPerKwh)} บาท/kWh`)
                    .join(", ")}
                </p>
              </div>
            </div>
                    ...
                    {touResult.intervalTraces.map((trace) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          {trace.isHoliday ? "ใช่" : "ไม่ใช่"}
                        </td>
                        <td className="px-3 py-2">
                          {formatKwh(trace.energyKwh)}
                        </td>
                        <td className="px-3 py-2">
                          {formatRate(trace.rateThbPerKwh)} บาท/kWh
                        </td>
                        <td className="px-3 py-2">
                          {formatThb(trace.energyChargeThb)} บาท
                        </td>
                      </tr>
                    ))}
                  </tbody>
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Badge
              variant={
                result.tariffStatus === "published" ? "success" : "warning"
              }
            >
              {translateStatus(result.tariffStatus)}
            </Badge>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        <div className="mb-4 grid gap-3 sm:grid-cols-4">
          <Summary
            label="พลังงาน"
            value={formatKwh(result.energyKwh)}
            unit="kWh"
          />
          <Summary
            label="รวมก่อนภาษี"
            value={formatThb(result.subtotalBeforeVat)}
            unit="บาท"
          />
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          สรุปอัตราค่าไฟสำหรับผู้ใช้ทั่วไป
        </CardTitle>
      </CardHeader>
      <CardContent className="grid gap-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <Summary
            label="ประเภทอัตราค่าไฟ"
            value={
              customerSegment === "small_business"
                ? "ธุรกิจขนาดเล็ก"
                : "บ้านอยู่อาศัย"
            }
            unit={result.mode === "tou" ? "ตามช่วงเวลา" : "อัตราปกติ"}
          />
          <Summary
            label="การไฟฟ้าที่ใช้"
            value={snapshot.authority}
            unit={
              snapshot.authority === "PEA"
                ? "การไฟฟ้าส่วนภูมิภาค"
                : "การไฟฟ้านครหลวง"
            }
          />
          ...
          <Summary
            ...
            value={
              ftPeriod
                ? `${formatRate(ftPeriod.ftThbPerKwh)} บาท/หน่วย`
                : "ไม่มีข้อมูล Ft"
            }
            unit={
              ftPeriod
                ? `มีผล ${formatRange(ftPeriod.effectiveFrom, ftPeriod.effectiveTo)}`
                : "ไม่ควรใช้สร้างผลลัพธ์"
            }
          />
          <Summary
            label="วันที่มีผล"
            value={formatRange(snapshot.effectiveFrom, snapshot.effectiveTo)}
            unit="อัตราค่าไฟฐาน"
          />
          <Summary
            label="ตรวจสอบล่าสุด"
            value={formatDate(snapshot.verifiedAt)}
            unit="ภาษีมูลค่าเพิ่มรวมอยู่ในยอดรวมแล้ว"
          />
        </div>
        <div className="grid gap-2 text-sm">
          <a
            ...
            rel="noreferrer"
          >
            ดูแหล่งอ้างอิงอัตราค่าไฟฐาน
          </a>
          {ftPeriod?.sourceUrl ? (
            <a
              className="w-fit text-primary underline underline-offset-4"
              href={ftPeriod.sourceUrl}
              target="_blank"
              rel="noreferrer"
            >
              ดูแหล่งอ้างอิงค่า Ft
            </a>
          ) : null}
        </div>
      </CardContent>
...
function getFtPeriod(result: TariffCalculationResult, billDate: string) {
  return result.tariffSnapshot.tariffVersion.ftPeriods.find(
    (period) =>
      billDate >= period.effectiveFrom &&
      (period.effectiveTo === null || billDate <= period.effectiveTo),
  );
}
...
function formatRange(from: string, to: string | null) {
  return `${formatDate(from)}${to ? ` ถึง ${formatDate(to)}` : " เป็นต้นไป"}`;
}
