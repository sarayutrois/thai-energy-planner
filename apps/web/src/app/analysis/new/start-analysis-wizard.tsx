...
const userTypes: Array<{
...
}> = [
  {
    value: "home",
    title: "บ้านพักอาศัย",
    description: "เหมาะกับเจ้าของบ้านที่อยากรู้ว่าค่าไฟสูงเพราะอะไร และ Solar/EV คุ้มไหม",
    icon: Home
  },
  {
    value: "shop",
    title: "ร้านค้า",
    description: "เหมาะกับร้านที่เปิดกลางวัน มีแอร์ ตู้แช่ หรือโหลดค่อนข้างสม่ำเสมอ",
    icon: Store
  },
  {
    value: "business",
    title: "ธุรกิจขนาดเล็ก",
    description: "เหมาะกับออฟฟิศ โกดัง หรือกิจการที่อยากเทียบ Normal, TOU และ Solar",
    icon: Building2
  }
];
...
const dataOptions: Array<{
...
}> = [
  {
    value: "bills",
    title: "มีบิลค่าไฟ",
    description: "เริ่มง่ายที่สุด กรอกบิลย้อนหลังเพื่อดูค่าไฟเฉลี่ยและแนวโน้มเบื้องต้น",
  ...
  },
  {
    ...
    description: "ใช้ข้อมูลละเอียดระดับเวลา เพื่อเทียบ TOU, Solar, Battery และ EV ได้แม่นขึ้น",
  ...
  },
  {
    ...
    description: "สร้างโหลดตัวอย่างจากเครื่องใช้ไฟฟ้า แล้วใช้ทดลองวิเคราะห์ก่อนได้",
    ...
    <>
        ...
        <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 py-8 md:px-6 lg:grid-cols-[1.02fr_0.98fr] lg:py-10">
          <div className="flex flex-col justify-center gap-6">
            ...
            <div className="space-y-4">
              ...
              <p className="max-w-2xl text-base leading-7 text-muted-foreground">
                เลือกว่าคุณเป็นผู้ใช้แบบไหน มีข้อมูลอะไรอยู่ แล้วระบบจะพาไปหน้าที่เหมาะที่สุด
          ...
          <Card className="shadow-panel">
            ...
            <CardContent className="grid gap-3">
              <StatusRow label="ประเภทผู้ใช้" value={userTypes.find((item) => item.value === audience)?.title ?? "บ้านพักอาศัย"} />
              <StatusRow label="คำแนะนำแรก" value="เริ่มจากข้อมูลที่มีอยู่จริงก่อน แล้วค่อยทดลอง scenario" />
              <StatusRow label="ข้อมูลจะถูกส่งต่อ" value="ลิงก์ถัดไปจะติด audience/source เพื่อให้หน้าปลายทางแนะนำต่อได้" />
            </CardContent>
          </Card>
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-8 md:px-6 lg:py-10">
        <div className="mb-5">
          ...
          <h2 className="mt-1 text-2xl font-semibold tracking-normal">เลือกประเภทผู้ใช้</h2>
        </div>
          ...
          {userTypes.map((item) => {
            ...
            return (
              <button
                ...
                type="button"
              >
                <Card className={`h-full transition ${selected ? "border-primary bg-primary/5" : "hover:border-primary"}`}>
                  <CardContent className="flex h-full flex-col gap-3 p-5">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-md bg-secondary text-secondary-foreground">
                        <item.icon aria-hidden="true" className="h-5 w-5" />
                      </div>
                      {selected ? <Badge variant="success">เลือกอยู่</Badge> : null}
                    </div>
                    <div>
      ...
      <section className="border-y border-border bg-white/78">
        <div className="mx-auto w-full max-w-7xl px-4 py-8 md:px-6 lg:py-10">
          <div className="mb-5">
            ...
            <h2 className="mt-1 text-2xl font-semibold tracking-normal">เลือกข้อมูลที่มีตอนนี้</h2>
          </div>
                ...
                <Card className="h-full transition hover:border-primary">
                  <CardHeader>
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Badge variant={item.badge === "แนะนำ" ? "success" : "outline"}>{item.badge}</Badge>
                    </div>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-8 md:px-6 lg:py-10">
        <div className="mb-5">
          ...
          <h2 className="mt-1 text-2xl font-semibold tracking-normal">ต่อยอดหลังมีข้อมูลแล้ว</h2>
        </div>
