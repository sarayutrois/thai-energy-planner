...
export function EnergyOverviewDashboard() {
  ...
  useEffect(() => { try { setProfile(readLocalLoadProfileSnapshot()); setBills(readStoredBillWorkspace()); } catch { setProfile(null); setBills(null); } }, []);
  ...
  return <div className="mt-6 grid gap-5">
    ...
    <section className="rounded-xl border border-border bg-card p-5"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><h2 className="text-xl font-semibold">ภาพรวมข้อมูลการใช้ไฟ</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">ตัวเลขจากบิลจะมีความสำคัญเหนือกว่า Load Profile ที่สร้างจากรายการเครื่องใช้ไฟฟ้า</p></div><Badge variant={usingSampleBills ? "warning" : profile ? "success" : "outline"}>{sourceQuality}</Badge></div>
      ...
      <div className="mt-4"><DataConfidence tone={profile?.canonicalProfile?.quality.level === "measured" ? "success" : profile ? "warning" : "information"} label={profile ? "ระดับความน่าเชื่อถือของผล" : "ยังไม่พร้อมวิเคราะห์เต็มรูปแบบ"} detail={profile ? "เพิ่มบิลค่าไฟย้อนหลังเพื่อเปรียบเทียบความใกล้เคียง และทำให้คำแนะนำเรื่อง TOU หรือ Solar น่าเชื่อถือขึ้น" : "เริ่มจากสร้างรูปแบบการใช้ไฟจากเครื่องใช้ไฟฟ้า หรือนำเข้าไฟล์จาก Smart Meter"} /></div>
    ...
    <div className="grid gap-5 lg:grid-cols-2"><SourceCard title="ข้อมูลจาก Load Profile" present={Boolean(profile)} lines={profile ? [`แหล่งข้อมูล: ${profile.sourceName}`, `จำนวนช่วงข้อมูล: ${profile.rowCount.toLocaleString("th-TH")}`, `อัปเดต: ${new Date(profile.updatedAt).toLocaleString("th-TH-u-ca-gregory", { dateStyle: "medium", timeStyle: "short" })}`] : ["กรุณาสร้างจากเครื่องใช้ไฟฟ้าหรือนำเข้าจากมิเตอร์"]} actionHref="/analysis/load-data" actionLabel={profile ? "แก้ไข Load Profile" : "เพิ่ม Load Profile"} /><SourceCard title="ข้อมูลจากบิลค่าไฟ" present={validBills.length > 0} lines={validBills.length ? [`จำนวนบิลที่ใช้: ${validBills.length} เดือน`, `เฉลี่ย: ${format(billMonthlyKwh ?? 0)} kWh / ${format(billMonthlyCost ?? 0)} บาท ต่อเดือน`, usingSampleBills ? "สถานะ: ข้อมูลตัวอย่าง ใช้ทดลอง flow เท่านั้น" : validBills.length >= 6 ? "ความน่าเชื่อถือ: ปานกลางถึงสูง" : "ความน่าเชื่อถือ: ควรเพิ่มบิลย้อนหลังอย่างน้อย 6 เดือน"] : ["ยังไม่มีข้อมูลจากบิล จึงยังตรวจสอบความใกล้เคียงของ Load Profile ไม่ได้"]} actionHref="/analysis/load-data/bills" actionLabel={validBills.length ? "แก้ไขข้อมูลบิล" : "เพิ่มข้อมูลบิล"} /></div>
...
function Metric({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <MetricCard icon={icon} label={label} value={value} />; }
