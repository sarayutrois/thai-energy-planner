...
import { deleteLocalAnalysisReport, readLocalAnalysisReports } from "@/lib/local-analysis-report";
        ...
        <Card key={report.id}>
          ...
          <CardContent className="grid gap-4">
            <div className="grid gap-3 md:grid-cols-3">
              <Metric label="สร้างเมื่อ" value={new Date(report.createdAt).toLocaleDateString("th-TH")} />
              <Metric label="บิลต้นทาง" value={`${report.sourceBill.monthCount} เดือน`} />
              <Metric label="คุณภาพข้อมูล" value={report.sourceBill.dataQualityLabel} />
            </div>
            <div className="flex flex-wrap gap-2">
              <a
