...
export type ReportReadinessStatus =
  "no_data" | "incomplete" | "low_confidence" | "ready";

export function getReportReadinessStatus(
  input: ReportReadinessInput,
): ReportReadinessStatus {
  if (!input.hasBills && !input.hasLoadProfile && !input.hasVerifiedResult)
    return "no_data";
  if (
    !input.hasBills ||
    !input.hasLoadProfile ||
    !input.hasScenario ||
    !input.hasSolar ||
    !input.hasTariffTrace ||
    !input.hasVerifiedResult
  )
    return "incomplete";
