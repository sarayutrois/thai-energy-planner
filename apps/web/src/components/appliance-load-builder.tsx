...
import {
  ...
  type ApplianceWorkspaceMode,
} from "@/components/appliance-workspace-state";
import {
...
const presets = [
  ...
  {
    name: "โทรทัศน์",
    category: "เครื่องใช้ไฟฟ้าภายในบ้าน",
    powerW: 100,
    dutyCycle: 1,
    start: "18:00",
    end: "22:00",
...
const homeStarterSets: Array<{
...
}> = [
  {
    ...
    items: [
      {
        name: "เครื่องปรับอากาศ Inverter 12,000 BTU",
        category: "เครื่องปรับอากาศ",
  ...
  },
  {
    ...
    description:
      "แอร์ 2 เครื่อง พร้อมปั๊มน้ำ เครื่องทำน้ำอุ่น และเครื่องใช้ทั่วไป",
    items: [
      {
        name: "เครื่องปรับอากาศ Inverter 9,000 BTU",
        category: "เครื่องปรับอากาศ",
...
function createDailySchedules(item: ApplianceInput): ApplianceScheduleInput[] {
  ...
  return days.flatMap((day) => {
    const source = sourceSchedules.find((schedule) =>
...
function scheduleForDay(
  item: ApplianceInput,
  day: number,
): ApplianceScheduleInput | undefined {
  const sourceSchedules = item.schedules?.length
    ? item.schedules
    : [item.schedule];
...
export function ApplianceLoadBuilder({
  ...
  const [mode, setMode] = useState<ApplianceWorkspaceMode>(
    initialAppliances.length ? "user" : "empty",
  );
    ...
    try {
      ...
      if (draft) {
        if (!hasUserEditedBeforeHydration.current) {
          const storedMode = storedApplianceWorkspaceMode(
            draft.mode,
  ...
  const validationIssues = useMemo(() => {
    ...
    if (!appliances.length)
      issues.push("เพิ่มเครื่องใช้ไฟฟ้าอย่างน้อย 1 รายการ");
    if (endDate < startDate)
      issues.push("วันที่สิ้นสุดต้องไม่อยู่ก่อนวันที่เริ่มต้น");
    appliances.forEach((item, index) => {
      if (!item.name.trim())
        issues.push(`รายการที่ ${index + 1}: ระบุชื่อเครื่องใช้ไฟฟ้า`);
      if (!(item.power > 0))
        issues.push(`รายการที่ ${index + 1}: กำลังไฟต้องมากกว่า 0 W`);
      if (!(item.quantity > 0))
        issues.push(`รายการที่ ${index + 1}: จำนวนต้องมากกว่า 0`);
      const hasScheduledDay = (
        item.schedules?.length ? item.schedules : [item.schedule]
      ).some((schedule) => schedule.daysOfWeek.length > 0);
      if (!hasScheduledDay)
        issues.push(`รายการที่ ${index + 1}: เลือกวันใช้งานอย่างน้อย 1 วัน`);
    });
    return issues;
  ...
  const quality =
    appliances.length >= 5
      ? {
          label: "ปานกลาง",
          score: 62,
          detail: "คำนวณจากรายการและช่วงเวลาที่ระบุ",
        }
      : {
          label: "ต่ำ",
          score: 38,
          detail: "ข้อมูลเครื่องใช้ไฟฟ้ายังมีจำนวนจำกัด",
        };
  const chartData = useMemo(
    () => buildHourlyChart(simulation?.intervals ?? []),
  ...
  function updateDaySchedule(
    ...
    setAppliances((current) =>
      current.map((item, itemIndex) => {
        ...
        const schedules = createDailySchedules(item).filter(
          (schedule) => schedule.daysOfWeek[0] !== day,
        );
        if (patch) {
          schedules.push({
  ...
  function copyDaySchedule(
    ...
    setAppliances((current) =>
      current.map((item, itemIndex) => {
        if (itemIndex !== index) return item;
        const source =
          scheduleForDay(item, sourceDay) ??
          item.schedules?.[0] ??
          item.schedule;
        const schedules = createDailySchedules(item).filter(
          (schedule) => !targetDays.includes(schedule.daysOfWeek[0] ?? -1),
        );
        for (const day of targetDays)
          schedules.push({ ...source, daysOfWeek: [day] });
        return { ...item, schedules };
      }),
    );
    setMode("user");
  ...
  function addHomeStarterSet(id: string) {
    ...
    if (
      appliances.length > 0 &&
      !window.confirm("เปลี่ยนเป็นชุดตัวอย่างใหม่และแทนที่รายการเดิมใช่หรือไม่")
    )
      return;
  ...
  function updateAirConditioner(
    ...
    if (!size) return;
    const power =
      compressorType === "inverter" ? size.inverterW : size.fixedSpeedW;
    update(index, {
  ...
  async function saveLoadProfile(navigateToBills = false) {
    ...
    if (
      mode === "sample" &&
      !window.confirm(
        "ชุดอุปกรณ์นี้ยังเป็นข้อมูลตัวอย่าง ผลลัพธ์เป็นเพียงตัวอย่าง ต้องการบันทึกต่อหรือไม่",
      )
    )
      return;
    ...
    const snapshot = saveLocalLoadProfileSnapshot({
      ...
      rows,
      warnings: [
        "ผลคำนวณจากข้อมูลเครื่องใช้ไฟฟ้าที่ผู้ใช้ระบุ ยังไม่ได้เปรียบเทียบกับบิลค่าไฟ",
      ],
      persist: false,
    });
    ...
    if (navigateToBills)
      window.location.assign("/analysis/load-data/bills?source=appliance");
  }
    ...
    <div className="grid gap-6">
      <div className="flex flex-col gap-3 rounded-xl border border-border bg-muted/35 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            ข้อมูลหน้านี้จะใช้ต่อในการเปรียบเทียบค่าไฟ ประเมิน Solar
            และจัดทำรายงาน
          </p>
        </div>
      ...
      {mode === "sample" ? (
        <div className="rounded-xl border border-warning bg-warning/10 p-4">
          <p className="font-semibold">
            กำลังใช้ชุดอุปกรณ์ตัวอย่าง — กรุณาปรับจำนวน กำลังไฟ
            และเวลาใช้งานให้ตรงกับสถานที่จริง
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            <Button onClick={useSampleAsStartingPoint}>
      ...
      <section className="rounded-2xl border border-border bg-card p-4 shadow-panel md:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              เลือกค่าตั้งต้นแล้วแก้กำลังไฟและเวลาใช้งานให้ตรงกับสถานที่จริง
            </p>
          </div>
              ...
              >
                ...
                {allPresets.map((preset) => (
                  <option key={preset.name} value={preset.name}>
                    {preset.name} · {preset.powerW.toLocaleString("th-TH")} W
                  </option>
                ))}
              </select>
            </label>
        ...
        <div className="mt-4 rounded-md border border-dashed border-primary/40 bg-primary/5 p-3">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            เพิ่มรายการพื้นฐานให้ครบก่อน แล้วแก้จำนวน กำลังไฟ
            และเวลาให้ตรงบ้านจริง
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            {homeStarterSets.map((starterSet) => (
              <Button
                ...
                variant="outline"
                disabled={!hydrated}
                className="h-auto min-h-10 flex-col items-start px-3 py-2 text-left"
                onClick={() => addHomeStarterSet(starterSet.id)}
              >
                <span>{starterSet.label}</span>
        ...
        {appliances.length === 0 ? (
          <div className="mt-5 rounded-lg border border-dashed border-border px-5 py-10 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              เพิ่มเครื่องใช้ไฟฟ้าทีละรายการ หรือเลือกชุดตัวอย่างเพื่อเริ่มต้น
            </p>
          </div>
        ) : (
              ...
              >
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.5fr_1fr_.7fr_auto]">
                  ...
                  <NumberField
                    label="กำลังไฟ (W)"
                    value={
                      item.powerUnit === "kW" ? item.power * 1000 : item.power
                    }
                    min={1}
                    step={1}
                    onChange={(power) =>
                  ...
                  <div className="mt-3 grid gap-3 rounded-md border border-primary/20 bg-primary/5 p-3 md:grid-cols-3">
                    <label className="grid gap-1 text-xs font-medium">
                      ...
                      <select
                        ...
                        value={item.coolingCapacityBtu ?? 9000}
                        onChange={(event) =>
                          updateAirConditioner(
                            index,
                            Number(event.target.value),
                            item.compressorType ?? "inverter",
                          )
                        }
                      >
                        {airConditionerSizes.map((size) => (
                          <option key={size.btu} value={size.btu}>
                            {size.btu.toLocaleString("th-TH")} BTU
                          </option>
                        ))}
                      </select>
                    </label>
                    <label className="grid gap-1 text-xs font-medium">
                      ...
                      <select
                        className="h-10 rounded-md border border-input bg-background px-3 text-sm"
                        value={item.compressorType ?? "inverter"}
                        onChange={(event) =>
                          updateAirConditioner(
                            index,
                            item.coolingCapacityBtu ?? 9000,
                            event.target.value as "inverter" | "fixed_speed",
                          )
                        }
                      >
                    ...
                    <div className="rounded-md bg-background px-3 py-2 text-xs leading-5 text-muted-foreground">
                      <strong className="text-foreground">
                        แหล่งกำลังไฟ:{" "}
                      </strong>
                      {powerSourceLabel(item.powerSource)}
                      <br />
                      ค่ามาตรฐานจาก BTU เป็นค่าเริ่มต้น ควรแก้ W
                      ตามฉลากเครื่องจริงเพื่อเพิ่มความแม่นยำ
                    </div>
                  </div>
                ) : null}
      ...
      <section className="rounded-2xl border border-border bg-card p-4 shadow-panel md:p-6">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Metric
            ...
            label="กำลังไฟสูงสุด"
            value={hasAppliances ? `${formatNumber(peakKw)} kW` : "—"}
          />
          <Metric
            icon={<Zap className="h-5 w-5" />}
            label="พลังงานต่อวัน"
            value={hasAppliances ? `${formatNumber(dailyKwh)} kWh/วัน` : "—"}
          />
          <Metric
            icon={<ReceiptText className="h-5 w-5" />}
            label="พลังงานต่อเดือน"
            value={
              hasAppliances ? `${formatNumber(monthlyKwh)} kWh/เดือน` : "—"
            }
          />
          <Metric
            ...
            detail={hasAppliances ? quality.detail : undefined}
          />
        </div>
        <div className="mt-5 h-[300px] rounded-lg border border-border p-3">
          {chartData.some((row) => row.loadKw > 0) ? (
            ...
            <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
              กราฟจะแสดงเมื่อข้อมูลเครื่องใช้ไฟฟ้าครบ
            </div>
          )}
        </div>
        <p className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
          <Clock3 className="h-4 w-4" />
          กราฟแสดงกำลังไฟเฉลี่ยรายชั่วโมงตามช่วงเวลาที่ผู้ใช้ระบุ
        </p>
      </section>
...
function buildHourlyChart(rows: LoadIntervalInput[]) {
  ...
  return buckets.map((bucket) => ({
    hour: bucket.hour,
    loadKw: bucket.count ? bucket.total / bucket.count : 0,
  }));
}
...
function isAirConditioner(item: ApplianceInput) {
  return (
    item.applianceKind === "air_conditioner" ||
    item.category === "เครื่องปรับอากาศ"
  );
}
